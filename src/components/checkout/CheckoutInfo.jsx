...
const CheckoutInfo = ({ cart, setCart, cartChange, setCartChange }) => {
  ...
  const handleOrder = async () => {
    ...
    const handleOrder = async () => {
      ...
      const newAddress = cart.address || `${ward}, ${district}, ${province}`;
      const totalPay =
        cart.orderDetails?.reduce(
          (total, item) => total + item.book.salePrice * item.amount,
          0
        ) + +cart.shippingPrice;
      console.log("Selected address data:", { newAddress });
      ...
      try {
        ...
        if (updateResponse.status === 200) {
          ...
          const response = await fetch(
            `http://localhost:3001/api/vnpay/url/${cart.id}`
          );
          const paymentUrl = await response.text();
          console.log("Payment URL:", paymentUrl);
          navigate(paymentUrl);
          ...
          message.error(
            "Cập nhật đơn hàng không thành công. Vui lòng thử lại."
          );
        }
      } catch (err) {
        console.error("Error updating order or adding order:", err);
        message.error(
          "Có lỗi xảy ra khi cập nhật đơn hàng hoặc thêm đơn hàng. Vui lòng thử lại."
        );
      }
    } else if (paymentMethod === "cod") {
      const newAddress = cart.address || `${ward}, ${district}, ${province}`;
